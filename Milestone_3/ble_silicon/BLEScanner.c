/**
 * @file BLEScanner.c
 * @brief BLE Scanner implementation for EFR32xG26 with Wirepas Protocol Controller interface
 *
 * This module implements a BLE scanner that:
 * 1. Scans for BLE advertisements in passive mode
 * 2. Encodes received advertisements in CBOR format
 * 3. Transmits the data using the Wirepas Protocol Controller (WPC) interface
 *
 * The implementation uses:
 * - Silicon Labs Bluetooth stack for BLE scanning
 * - CBOR encoding for data serialization
 * - WPC protocol for communication with the Wirepas network stack
...
// WPC protocol constants
#define WPC_FRAME_DELIMITER 0xFF
#define WPC_ACK_TIMEOUT_MS 100  // Timeout for acknowledgment

/**
 * @brief Error codes for WPC operations
 */
typedef enum {
    WPC_OK = 0,                      // Operation successful
    WPC_ERROR_INVALID_PARAMS = -1,   // Invalid parameters provided
    WPC_ERROR_PDU_TOO_LARGE = -2,    // PDU size exceeds maximum allowed
    WPC_ERROR_SERIAL_WRITE = -3,     // Error writing to serial port
    WPC_ERROR_NO_ACK = -4,          // No acknowledgment received
    WPC_ERROR_INVALID_ACK = -5       // Invalid acknowledgment received
} wpc_error_t;
